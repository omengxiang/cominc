...
#if ((defined(_M_IX86) && !defined(_M_HYBRID)) || defined(_M_X64)) && !(defined(_CRT_APP) || defined(_KERNELX))
...
_CRTIMP2 unsigned int __cdecl GetNextAsyncId()
{
    ...
    return static_cast<unsigned int>(::_InterlockedIncrement(reinterpret_cast<volatile LONG *>(&s_asyncId)));
        ...
        _CRTIMP2 void __thiscall _ContextCallback::_CallInContext(_CallbackFunction _Func, bool _IgnoreDisconnect) const
        {
...
#ifdef _CRT_APP
    _CRTIMP2 __thiscall task_continuation_context::task_continuation_context() : _ContextCallback(true), _M_RunInline(false)
    {
    }
#else
    _CRTIMP2 __thiscall task_continuation_context::task_continuation_context() : _ContextCallback(false), _M_RunInline(false)
    {
    }
#endif
} // namespace Concurrency
