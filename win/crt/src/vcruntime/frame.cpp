...
#define __GetRangeOfTrysToCheck(a, b, c, d, e, f, g) \
                                RENAME_EH_EXTERN(_GetRangeOfTrysToCheck)(b, c, d, e, f)

#define __FrameUnwindToEmptyState(a, b, c) \
                                RENAME_EH_EXTERN(__FrameUnwindToState)(a, b, c, EH_EMPTY_STATE);

#define __GetUnwindState(a, b, c) GetCurrentState(a, b, c)
...
#define __GetRangeOfTrysToCheck(a, b, c, d, e, f, g) \
                                RENAME_EH_EXTERN(_GetRangeOfTrysToCheck)(a, b, c, d, e, f, g)

#define __GetUnwindState(a, b, c) \
                                RENAME_EH_EXTERN(__GetCurrentState)(a, b, c)
    ...
    if (IS_UNWINDING(PER_FLAGS(pExcept)))
    {
        ...
        if (FUNC_MAXSTATE(*pFuncInfo) != 0 && CatchDepth == 0)
        {
            ...
            if (IS_TARGET_UNWIND(PER_FLAGS(pExcept)) && PER_CODE(pExcept) == STATUS_LONGJUMP) {
                    __ehstate_t target_state = RENAME_EH_EXTERN(__StateFromIp)(
                                                             pFuncInfo,
                                                             pDC,
#if defined(_M_X64)
                                                             pContext->Rip
                                                             ...
                                                             pContext->Eip      // HYB-TODO: Figure out which type of context will be passed here.
#endif
                                                             );
                    ...
                    RENAME_EH_EXTERN(__FrameUnwindToState)(pRN, pDC, pFuncInfo, target_state);
                      ...
                      PER_CODE(pExcept) == STATUS_UNWIND_CONSOLIDATE)
            {
                ...
                DASSERT(target_state >= EH_EMPTY_STATE
                        && target_state < FUNC_MAXSTATE(*pFuncInfo));
                RENAME_EH_EXTERN(__FrameUnwindToState)(
                                    (EHRegistrationNode *)pSehExcept->ExceptionInformation[1],
                                    pDC,
                                    pFuncInfo,
                                    target_state);
                EHTRACE_HANDLER_EXIT(ExceptionContinueSearch);
            ...
            RENAME_EH_EXTERN(__FrameUnwindToEmptyState)(pRN, pDC, pFuncInfo);
        }
        EHTRACE_HANDLER_EXIT(ExceptionContinueSearch);
...
)
{
    ...
    RENAME_EH_EXTERN(_GetEstablisherFrame)(pRN, pDC, pFuncInfo, &EstablisherFrame);
    if (curState > RENAME_EH_EXTERN(__GetUnwindTryBlock)(pRN, pDC, pFuncInfo)) {
        RENAME_EH_EXTERN(__SetState)(&EstablisherFrame, pDC, pFuncInfo, curState);
        RENAME_EH_EXTERN(__SetUnwindTryBlock)(pRN, pDC, pFuncInfo, /*curTry*/ curState);
    } else {
        curState = RENAME_EH_EXTERN(__GetUnwindTryBlock)(pRN, pDC, pFuncInfo);
    }
#else
            ...
            for (; curTry < end; curTry++, pEntry++) {
                ...
                pCatch++) {
                    ...
                    catchables > 0; catchables--, ppCatchable++) {
                        ...
                        if (!RENAME_EH_EXTERN(__TypeMatch)(pCatch, pCatchable, PER_PTHROW(pExcept))) {
                            continue;
                        }
    ...
    NextTryBlock: ;
        ...
        if( !gotMatch && FUNC_MAGICNUM(*pFuncInfo) >= EH_MAGIC_HAS_ES &&
            (FUNC_PESTYPES(pFuncInfo) != nullptr || (((FUNC_FLAGS(*pFuncInfo) & FI_EHNOEXCEPT_FLAG) != 0) &&
                ...
                CatchDepth == 0
#else
                !RENAME_EH_EXTERN(_ExecutionInCatch)(pDC, pFuncInfo)
#endif
            )))
        {
            // Are we noexcept?
            ...
            if( !IsInExceptionSpec(pExcept, FUNC_PESTYPES(pFuncInfo)) )
            {
                ...
                pEstablisher = RENAME_EH_EXTERN(_GetEstablisherFrame)(pRN, pDC, pFuncInfo, &EstablisherFramePointers);
                PVOID pExceptionObjectDestroyed = nullptr;
                RENAME_EH_EXTERN(_UnwindNestedFrames)(pRN,
                                    pExcept,
                                    pContext,
                                    pEstablisher,
                                    nullptr,
...
)
{
    ...
    if (__pSETranslator != nullptr && __pSETranslator != __vcrt_EncodePointer(nullptr) &&
      ...
      pExcept->ExceptionCode != MANAGED_EXCEPTION_CODE_V4) {
        ...
        if (RENAME_EH_EXTERN(_CallSETranslator)(pExcept, pRN, pContext, pDC, pFuncInfo,
                                                CatchDepth, pMarkerRN)) {
            EHTRACE_EXIT;
            return;
        }
...
#endif
#if defined(_M_X64) || defined(_M_ARM_NT) || defined(_M_ARM64) || defined(_CHPE_X86_ARM64_EH_)
    __ehstate_t curState = RENAME_EH_EXTERN(__GetCurrentState)(pRN, pDC, pFuncInfo);
#else
...
#endif
    {
        ...
        __try {
            ...
            if (UWE_ACTION(FUNC_UNWIND(*pFuncInfo, curState)) != nullptr) {
                ...
                // already completed:
#if defined(_M_X64) || defined(_M_ARM_NT) || defined(_M_ARM64) || defined(_CHPE_X86_ARM64_EH_)
                RENAME_EH_EXTERN(__SetState)(pRN, pDC, pFuncInfo, nxtState);
#else
...
#if defined(_M_X64) || defined(_M_ARM_NT) || defined(_M_ARM64) || defined(_CHPE_X86_ARM64_EH_)
    RENAME_EH_EXTERN(__SetState)(pRN, pDC, pFuncInfo, curState);
#else
...
static void CatchIt(
    ...
    if (pConv != nullptr) {
        RENAME_EH_EXTERN(__BuildCatchObject)(pExcept, pEstablisher, pCatch, pConv);
    }
    ...
    RENAME_EH_EXTERN(_UnwindNestedFrames)(
        pRN,
        pExcept,
        pContext,
        pEstablisher,
    ...
    RENAME_EH_EXTERN(__FrameUnwindToState)(pEstablisher, pDC, pFuncInfo, TBME_LOW(*pEntry));
...
extern "C" void * RENAME_EH_EXTERN(__CxxCallCatchBlock)(
...
static __declspec(guard(ignore)) void *CallCatchBlock(
    ...
    __try {
        __try {
            ...
            RENAME_EH_EXTERN(__FrameUnwindToState)(pRN, nullptr, pFuncInfo, cState);
...
extern "C" _VCRTIMP void __cdecl RENAME_EH_EXTERN(__BuildCatchObject)(
    ...
    __try {
        switch(RENAME_EH_EXTERN(__BuildCatchObjectHelper)(pExcept, pRN, pCatch, pConv))
        {
            case 1:
