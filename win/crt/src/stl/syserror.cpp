...
_CRTIMP2_PURE unsigned long __CLRCALL_PURE_OR_CDECL _Winerror_message(
	...
	{	// convert to name of Windows error, return 0 for failure,
		// otherwise return number of chars written, excluding null terminator
	wstring _Wide(_Size, L'\0');

	if (FormatMessageW(
		FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
		0, _Message_id, 0, &_Wide[0], _Size, 0) == 0)
		return (0);

	const int _Bytes = WideCharToMultiByte(
		CP_ACP, 0, &_Wide[0], -1, _Narrow, _Size, 0, 0);

	if (_Bytes == 0)
		return (0);

	return (static_cast<unsigned long>(_Bytes - 1));
	}
