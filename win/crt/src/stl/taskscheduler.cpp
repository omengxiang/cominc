...
// the DLL in which the callback is located.
        ...
        HMODULE _Call_get_module_handle_ex(DWORD _Flags, LPCWSTR _Addr)
            ...
            (void)_Addr;
            return 0;
#else /* ^^^ defined(_CRT_APP) && !defined(_KERNELX) ^^^ // vvv !defined(_CRT_APP) || defined(_KERNELX) vvv */
            HMODULE _Result;
        ...
        struct _Task_scheduler_main_block
            ...
            ~_Task_scheduler_main_block() _NOEXCEPT
                {   // block shutdown of the CRT until std::async shutdown has completed
        ...
        void CALLBACK _Task_scheduler_callback(PTP_CALLBACK_INSTANCE _Pci, PVOID _Args, PTP_WORK) _NOEXCEPT
            {
            _Increment_outstanding();
            const auto _Chore = static_cast<_Threadpool_chore *>(_Args);
            if (_Get_STL_host_status() != _STL_host_status::_Exe)
                {   // ensure user code held alive by _Reschedule_chore is freed when we're done
        ...
        DWORD __stdcall _Task_scheduler_callback_xp(LPVOID _Args) _NOEXCEPT
            {
            _Increment_outstanding();
            const auto _Chore = static_cast<_Threadpool_chore *>(_Args);
            _Chore->_M_callback(_Chore->_M_data);
    ...
    _CRTIMP2 int __cdecl _Schedule_chore(_Threadpool_chore *_Chore)
        ...
        if (_Is_vista_threadpool_supported())
            ...
            {
            // XP supporting
            // XP doesn't support FreeLibraryWhenCallbackReturns, so we prevent the callback DLL from ever unloading
            if (_Get_STL_host_status() != _STL_host_status::_Exe)
                {
                (void)_Call_get_module_handle_ex(
