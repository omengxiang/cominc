...
static const pcchar_t tokenTable[] =
{
#if !VERS_32BIT
	"__near",		// TOK_near
	"__near ",		// TOK_nearSp
...
static const pcchar_t nameTable[] =
{
	" new",
	" delete",
	"=",
...
private:
	...
	void append(DNameNode *newRight);
...
private:
	...
	static DName getDataIndirectType(const DName &, const char*, const DName &, int = FALSE);
	static DName getExtendedDataIndirectType(const char*& prType, bool& fIsPinPtr, int thisFlag);
	static DName getDataIndirectType();
	static DName getBasicDataType(const DName &);
	static DName getECSUDataType(void);
	static DName getPtrRefType(const DName &, const DName &, const char*);
	...
	static DName getReferenceType(const DName &, const DName &, const char*);
...
DName UnDecorator::getStringEncoding(_In_z_ const char *prefix, int /*unused*/)
{
	DName result = prefix;
				...
				// This is new style of enclosing any interface whose method the class is implementing
			{
				...
				do
				{
					...
					if (scopeName.status() == DN_valid)
					{
						...
						{
							explicitScope = scopeName + "::" + explicitScope;
						}
						else
						{
...
DName UnDecorator::getTemplateConstant(void)
{
	...
	case TC_fp:
	{
		...
		if (mantissa.isValid() && exponent.isValid())
		{
			...
			if (!mantissa.getString(buf + 1, _countof(buf) - 1))
			{
				return DN_invalid;
			}
	...
	case TC_generic_method_parameter:
	{
		...
		case TC_template_type_parameter:
		case TC_template_type_parameter_pack:
			return "`template-type-parameter-" + index + "'";
			break;
		case TC_generic_class_parameter:
			return "`generic-class-parameter-" + index + "'";
			break;
		case TC_generic_method_parameter:
			return "`generic-method-parameter-" + index + "'";
			break;
		}
...
inline DName UnDecorator::composeDeclaration(const DName & symbol)
{
...

	{
...
#endif // !NO_COMPILER_NAMES
		{
			...
			{
				if (TE_isvtoradjex(typeCode))
					declaration += "`vtordispex{" + vbptrDisp + ',' + vbindex + ',' + vtorDisp + ',';
				else if (TE_isvtoradj(typeCode))
...
DName UnDecorator::getScopedName(void)
{
	...
	if (*gName == '@')
		...
		scopeName = DName(DN_truncated) + "::" + scopeName;
...
DName UnDecorator::getDataType(DName * pDeclarator)
{
	...
	case '?':
	{
		...
		superType = getDataIndirectType(superType, pStr, DName(), 0);
...
DName UnDecorator::getPrimaryDataType(const DName & superType)
{
	...
	case PDT_reference:
	{
		...
		return getReferenceType(cvType, superName.setPtrRef(), pStr);
	...
	case PDT_extend:
	{
		...
		case PDT_ex_qualified:
		{
			...
			return getBasicDataType(getDataIndirectType(superType, pStr, DName(), 0));
		}
		break;
		...
		case PDT_ex_nullptr_t:
			...
			if (!superType.isEmpty())
			{
				return "std::nullptr_t " + superType;
			}
			else
			{
...
DName UnDecorator::getArgumentTypes(void)
{
	...
	{
	case AT_ellipsis:
		return (gName++, doEllipsis() ? "..." : UNDNAME_ELLIPSIS_ALTERNATE);
	...
	default:
	{
		...
		if (arguments.status() == DN_valid)
			...
			case AT_ellipsis:
				return (gName++, arguments + (doEllipsis() ? ",..." : "," UNDNAME_ELLIPSIS_ALTERNATE));
	...
	if (*gName)
	{
		...
		case BDT_extend:
			...
			case BDT_unknown:
				...
				break;
			case BDT_char16_t:
				basicDataType = "char16_t";
				break;
			case BDT_char32_t:
				basicDataType = "char32_t";
				break;
			case BDT_wchar_t:
...
DName UnDecorator::getPtrRefType(const DName & cvType, const DName & superType, const char* ptrChar)
{
	DASSERT((ptrChar != NULL) && ((*ptrChar == '*') || (*ptrChar == '&') || (*ptrChar == '^') || (*ptrChar == '%') || (*ptrChar == '\0')));
...
DName UnDecorator::getExtendedDataIndirectType(const char*& prType, bool& fIsPinPtr, int thisFlag)
{
	DASSERT((prType != NULL) && ((*prType == '*') || (*prType == '&') || (*prType == '^') || (*prType == '%') || (*prType == '\0')));
...
DName UnDecorator::getDataIndirectType(const DName & superType, const char* prType, const DName & cvType, int thisFlag)
{
	DASSERT((prType != NULL) && ((*prType == '*') || (*prType == '&') || (*prType == '^') || (*prType == '%') || (*prType == '\0')));
	...
	if (*gName)
	{
		if (gName[0] == '$')
		{
			DName result = getExtendedDataIndirectType(prType, bIsPinPtr, thisFlag);
...
inline DName UnDecorator::getDataIndirectType()
{
	...
	return getDataIndirectType(DName(), pStr, DName());
}
...
inline DName UnDecorator::getThisType()
{
	...
	return getDataIndirectType(DName(), pStr, DName(), TRUE);
}
...
inline DName UnDecorator::getPointerType(const DName& cv, const DName& superType)
{
	...
	return getPtrRefType(cv, superType, pStr);
}
...
inline DName UnDecorator::getReferenceType(const DName& cv, const DName& superType, const char* ptrChar)
{
	return getPtrRefType(cv, superType, ptrChar);
}
