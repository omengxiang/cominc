...
namespace Concurrency
{
    ...
    {
        // This API should not be called by modern APPs or _KERNELX apps
...
namespace details
{
    ...
    unsigned int ResourceManager::GetCurrentNodeAndCore(unsigned int * pCore)
    {
        ...
        // For both _KERNELX and MSDK we assume a single node
        if (pCore != NULL)
        {
            *pCore = 0;
        }
                ...
                ::Concurrency::GetOSVersion() == ::Concurrency::IResourceManager::Vista)
        {
            ...
            DWORD processorNumber = __crtGetCurrentProcessorNumber();
            ULONG_PTR processorAffinity = static_cast<ULONG_PTR>(1) << processorNumber;
    ...
    {
        // MSDK compliant and _KERNELX applications are not allowed to create multiple schedulers.
...
#if (defined(_ONECORE) || defined(_KERNELX))
        SYSTEM_INFO sysInfo;
// TODO - KERNELX - We should be able to get rid if this next ifdef when KernelX introduces GetNativeSystemInfo in place of GetSystemInfo
#if defined(_KERNELX)
...
// TODO - KERNELX - We should be able to get rid if this next ifdef when KernelX introduces GetNativeSystemInfo in place of GetSystemInfo
        ...
        m_pGlobalNodes[0].Initialize(this, 0, 0, sysInfo.dwActiveProcessorMask);
