...
_FS_DLL TCHAR *__CLRCALL_PURE_OR_CDECL _Read_dir(
	TCHAR (&_Dest)[_MAX_FILESYS_NAME], void *_Handle, file_type& _Ftype)
...
_FS_DLL file_type __CLRCALL_PURE_OR_CDECL _Stat(const TCHAR *_Fname, perms *_Pmode)
	{	// get file status
...
_FS_DLL file_type __CLRCALL_PURE_OR_CDECL _Lstat(const TCHAR *_Fname, perms *_Pmode)
	{	// get symlink file status
	return (_Stat(_Fname, _Pmode));	// symlink not supported
	}

_FS_DLL unsigned long long __CLRCALL_PURE_OR_CDECL _Hard_links(const TCHAR *_Fname)
	{	// get hard link count
	...
	return (_Ok ? _Info.NumberOfLinks : (unsigned long long)-1);
	...
	return (_Ok ? _Info.nNumberOfLinks : (unsigned long long)-1);
  #endif /* defined(_CRT_APP) */
	}


_FS_DLL unsigned long long __CLRCALL_PURE_OR_CDECL _File_size(const TCHAR *_Fname)
	{	// get file size
	WIN32_FILE_ATTRIBUTE_DATA _Data;
	...
	if (!TFUN(GetFileAttributesEx)(_Fname, GetFileExInfoStandard, &_Data))
		...
		return ((unsigned long long)_Data.nFileSizeHigh << 32 | _Data.nFileSizeLow);
	}
...
_FS_DLL int64_t __CLRCALL_PURE_OR_CDECL _Last_write_time(const TCHAR *_Fname)
	...
	if (!TFUN(GetFileAttributesEx)(_Fname, GetFileExInfoStandard, &_Data))
		...
		unsigned long long _Wtime =
			(unsigned long long)_Data.ftLastWriteTime.dwHighDateTime << 32
...
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Set_last_write_time(const TCHAR *_Fname,
	...
	unsigned long long _Wtime = (unsigned long long)_When + WIN_TICKS_FROM_EPOCH;
	FILETIME _Ft;
...
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Chmod(const TCHAR *_Fname, perms _Newmode)
